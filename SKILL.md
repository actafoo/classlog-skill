---
name: classlog-app
description: 교사가 만들고 싶은 어떤 HTML 학습활동이든(퀴즈·게임·방탈출·연습·체험) 형태 그대로 만들되, 그 안의 학습 신호를 ClassLog(교사용 형성평가 플랫폼)가 수합할 수 있는 데이터로 번역해 얹는 어댑터 스킬. 새로 만들 때는 교사에게 먼저 질문하고, 기존 HTML은 분석 후 수술적으로만 수정한다. 활동의 자유는 교사에게, 규격은 데이터에만.
---
...
## 대원칙 — 자유는 교사에게, 규격은 데이터에만

이 스킬은 앱의 **형태·디자인·아이디어를 제한하지 않습니다.** 퀴즈, 게임, 방탈출, 시뮬레이션,
무한 연습 드릴, 글쓰기 활동 — 교사가 원하는 것은 무엇이든, 원하는 모습 그대로 만드세요.
스킬의 역할은 단 하나입니다: **그 활동 안에서 일어나는 학습의 순간을 ClassLog가 수합할 수 있는
데이터(item / setScore / done)로 번역해 얹는 것.**

- 교사의 아이디어와 이 규격이 충돌하는 것처럼 보이면 **항상 교사의 아이디어가 이깁니다.**
  활동을 바꾸지 말고, "이 활동이라면 이렇게 기록할 수 있어요"라고 매핑을 제안하세요.
- 하지 말 것: 연동을 이유로 활동을 퀴즈 모양으로 바꾸자고 제안하기, 디자인을 단순화하기,
  아래 예제 코드의 생김새를 따라 하기. 예제는 API 위치를 보여주는 샘플일 뿐입니다.

### 어떤 활동이든 매핑됩니다 — 제안 가이드

| 교사가 만들고 싶은 것 | ClassLog 데이터로 이렇게 |
|---|---|
| 문제 풀이·퀴즈 | 문항마다 `item()`, 끝에 `done()` |
| 게임 (스테이지·미션·보스전) | 스테이지 성공/실패 = `item({ text: "스테이지 3: ○○ 미션" })`, 게임 점수 = `setScore()` |
| 무한 연습 드릴 | 시도마다 `item()` (카운터 id), 누적 점수 `setScore()`, `done()` 생략 |
| 방탈출·퍼즐 | 퍼즐 하나 = `item()` (푼 시도 내용을 response에) |
| 자유 입력 활동 (글쓰기, 답 설명하기) | 채점 없이 `item({ response: 학생이 쓴 것 })` — correct 생략 |
| 체험·시뮬레이션 (채점 없음) | 끝 지점에서 `done()` 하나 — 완료 기록 |

매핑이 애매하면 교사에게 물어보세요: **"이 활동에서 나중에 확인하고 싶은 게 뭔가요?"**
— 그 답이 무엇을 `item`으로 만들지 알려줍니다.
...
이미 답이 나온 항목은 다시 묻지 않습니다. 질문의 목적은 활동을 규격에 맞추는 게 아니라,
**교사가 상상하는 활동을 정확히 파악해서 그대로 만들어주는 것**입니다.

1. **무엇을 배우는 활동인가요?** — 학년, 과목, 학습 주제 (예: 초3 수학, 분수의 덧셈)
2. **어떤 모습의 활동을 상상하세요?** — 퀴즈, 게임, 방탈출, 카드 뒤집기, 무한 연습, 체험형…
   무엇이든 됩니다. 교사가 말한 형태를 그대로 존중하세요.
3. **학습 내용 재료가 있나요?** — 수업 자료, 교과서 단원, 예시 문제, 출제 범위가 있으면 붙여넣어
   달라고 요청하세요. **재료 없이 AI가 내용을 지어내면 교육과정과 어긋날 수 있습니다.** 재료가
   없다면: 만들 내용(문제·미션과 정답)을 먼저 목록으로 보여주고 교사의 확인을 받은 뒤 코드를 만드세요.
4. **나중에 뭘 확인하고 싶으세요?** — 이 답으로 매핑을 정하고, ClassLog 업로드 시 고를
   '기록 방식'을 안내하세요: 점수가 나오는 활동=**퀴즈형**, 끝없이 반복=**연습형**, 완료만=**완료형**.
...
**주의: 아래는 API를 어디서 호출하는지 보여주는 최소 샘플일 뿐입니다.**
실제 앱의 장르·규모·디자인은 교사의 요구를 따르세요. 게임을 원하면 게임을,
방탈출을 원하면 방탈출을 만들고, 그 안에 같은 호출만 얹으면 됩니다.
...
- [ ] **교사가 원한 활동의 형태를 연동 때문에 바꾸지 않았는가?** (퀴즈로 눌러 펴기 금지)
- [ ] (새로 만들 때) 학년·주제·내용 재료를 **물어봤는가?** 재료 없이 지어낸 내용은 확인받았는가?
